...
__global__ void wavelet_rasterize_backward_sum_kernel(
    const dim3 tile_bounds,
    const dim3 img_size,
    const int32_t* __restrict__ gaussian_ids_sorted,
    const int2* __restrict__ tile_bins,
    ...
    const float3* __restrict__ conics,                                   
    const float3* __restrict__ rgbs,
    const float2* __restrict__ frequencies, // frequencies
    const float* __restrict__ opacities,
    const float3& __restrict__ background,
    const float* __restrict__ final_Ts,
    const int* __restrict__ final_index,
    ...
    float3* __restrict__ v_conic,
    float3* __restrict__ v_rgb,
    float2* __restrict__ v_freqs, // gradient of frequencies
    float* __restrict__ v_opacity
) {
    auto block = cg::this_thread_block();
    int32_t tile_id =
    ...
    __shared__ float3 conic_batch[BLOCK_SIZE];
    __shared__ float3 rgbs_batch[BLOCK_SIZE];
    __shared__ float2 f_batch[BLOCK_SIZE]; // frequencies share cache
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range.x) {
            ...
            conic_batch[tr] = conics[g_id];
            rgbs_batch[tr] = rgbs[g_id];
            f_batch[tr] = frequencies[g_id]; // load frequencies
        }
        // wait for other threads to collect the gaussians in batch
        block.sync();
        // process gaussians in the current batch for this pixel
        ...
        for (int t = max(0,batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if(valid){
                ...
                sigma = 0.5f * (conic.x * delta.x * delta.x +
                                    conic.z * delta.y * delta.y) +
                                        conic.y * delta.x * delta.y;
                float modulator = 0.5f * (__cosf(f.x * delta.x + f.y * delta.y) + 1.0f);
                vis = modulator * __expf(-sigma);
                alpha = min(1.f, opac * vis);
                if (sigma < 0.f || alpha < 1.f / 255.f) {
                    valid = 0;
                }
            ...
            if(valid){
                ...
                float dmod_dx = -0.5f * __sinf(f.x * delta.x + f.y * delta.y) * f.x;
                float dmod_dy = -0.5f * __sinf(f.x * delta.x + f.y * delta.y) * f.y;
                v_xy_local = {v_sigma * (conic.x * delta.x + conic.y * delta.y) + opac * __expf(-sigma) * v_alpha * dmod_dx, 
                                    v_sigma * (conic.y * delta.x + conic.z * delta.y) + opac * __expf(-sigma) * v_alpha * dmod_dy};


                v_opacity_local = vis * v_alpha;

                v_freqs_local = {-opac * __expf(-sigma) * v_alpha * 0.5f * __sinf(f.x*delta.x + f.y*delta.y) * delta.x,
                                -opac * __expf(-sigma) * v_alpha * 0.5f * __sinf(f.x*delta.x + f.y*delta.y) * delta.y};
            }
            warpSum3(v_rgb_local, warp);
            warpSum3(v_conic_local, warp);
            warpSum2(v_xy_local, warp);
            warpSum(v_opacity_local, warp);
            warpSum2(v_freqs_local, warp);
            if (warp.thread_rank() == 0) {
                int32_t g = id_batch[t];
                float* v_rgb_ptr = (float*)(v_rgb);
                atomicAdd(v_rgb_ptr + 3*g + 0, v_rgb_local.x);
                ...
                float* v_freqs_ptr = (float*)(v_freqs);
                atomicAdd(v_freqs_ptr + 2*g + 0, v_freqs_local.x);
                atomicAdd(v_freqs_ptr + 2*g + 1, v_freqs_local.y);
            }
        }
    }
}
