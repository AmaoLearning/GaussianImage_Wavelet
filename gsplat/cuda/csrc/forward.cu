...
__global__ void wavelet_rasterize_forward_sum(
    ...
    const float3* __restrict__ conics,
    const float3* __restrict__ colors,
    const float2* __restrict__ frequencies, // 2D frequency vector (fx, fy) per primitive
    const float* __restrict__ opacities,
    float* __restrict__ final_Ts,
    int* __restrict__ final_index,
    float3* __restrict__ out_img,
    ...
    __shared__ float3 xy_opacity_batch[BLOCK_SIZE];
    __shared__ float3 conic_batch[BLOCK_SIZE];
    __shared__ float2 f_batch[BLOCK_SIZE]; // frequency shared cache
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            const float2 f = f_batch[t];
            const float modulator = 0.5f * (__cosf(f.x * delta.x + f.y * delta.y) + 1.0f);
            const float alpha = min(1.f, opac * modulator * __expf(-sigma));
