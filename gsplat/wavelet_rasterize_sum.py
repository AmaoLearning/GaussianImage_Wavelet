...
def wavelet_rasterize_gaussians_sum(
    ...
    frequencies: Float[Tensor, "*batch 2"],
    ...
    Args:
        ...
        frequencies (Tensor): xy freqs of 2D Gaussians.
    ...
    if frequencies.ndimension() != 2 or frequencies.size(1) != 2:
        raise ValueError("frequencies must have dimensions (N, 2)")

    return _WaveletRasterizeGaussiansSum.apply(
        xys.contiguous(),
        depths.contiguous(),
        radii.contiguous(),
        conics.contiguous(),
...
class _WaveletRasterizeGaussiansSum(Function):
    ...
    def forward(
        ...
        if num_intersects < 1:
            ...
            if colors.shape[-1] == 3:
                rasterize_fn = _C.wavelet_rasterize_sum_forward
            else:
    ...
    def backward(ctx, v_out_img, v_out_alpha=None):
        ...
        if num_intersects < 1:
            ...
            if colors.shape[-1] == 3:
                rasterize_fn = _C.wavelet_rasterize_sum_backward
            else:
                rasterize_fn = _C.nd_rasterize_sum_backward
            v_xy, v_conic, v_colors, v_freqs, v_opacity = rasterize_fn(
                img_height,
                img_width,
                BLOCK_H,
                BLOCK_W,
